...
class Counters(tk.LabelFrame):
    def __init__(self, master, count1, count2, p_reset, c_reset):
        ...
        self.c_count = tk.IntVar()
        self.c_count.set(count1)
        self.p_count = tk.IntVar()
        self.p_count.set(count2)
        round_counted = tk.LabelFrame(self, text="Rounds")
        counted_rounds = tk.Label(round_counted, textvariable = self.c_count, background="white")
        counted_rounds.pack(fill="both")
        round_counted.grid(column=0, row=0)
        primer_counter = tk.LabelFrame(self, text="Primer")
        primers_counted = tk.Label(primer_counter, textvariable = self.p_count, background="white")
        primers_counted.pack(fill="both")
        primer_counter.grid(column=1, row=0)
        primer_reset_button = ttk.Button(self,text="Primer", command = p_reset)
        primer_reset_button.grid(column=1, row=1)
        case_reset_button = ttk.Button(self,text="Case", command = c_reset)
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.bang_pills = Round_Var()
        self.bang_buttons = Primer_Var()
...
#Middle Container:
  ...
  #Second Frame:  
    #Counter        
        self.counter_box = Counters(self.middle_frame, count1=self.bang_pills,
                                     count2=self.bang_buttons,
                                     c_reset=self.bang_pills.Reset_Cases,
                                     p_reset=self.bang_buttons.Reset_Primers)
...
#Commands
        ...
        S2.when_pressed = self.bang_pills.Reset_Cases
        S3.when_pressed = self.bang_buttons.Reset_Primers
    ...
    def S1_Counted(self):
        ...
        M2.pulse(n=1)
        #Triggers Count
        self.bang_buttons.Round_Ticker()
        self.bang_buttons.Primer_Ticker()
